export const GAME = {
  ...
  GAP_REDUCTION: 6,                    // 通過ごとに隙間が何px狭くなるか（決定論的な基準幅に適用）
  GAP_MIN_MARGIN: 10,                  // 隙間幅の下限（ボール直径+この値）。これ以下にはならない
  // ステージ毎の幅ゆらぎ。決定論的な基準幅(gapBase)に±この範囲を毎回ふり直して乗せる。
  // 累積しない（次ステージは基準幅から計算）ので難易度カーブ自体は崩れず、単調さだけ消える。
  // 落下中に隙間が見えるので事前に読める＝フェア。floor/上限でクランプし理不尽スパイクを防ぐ。
  GAP_JITTER_PX: 8,                    // ステージ毎の幅ゆらぎ幅（±px）。0でゆらぎ無効
  LINE_SEGMENT_MIN_WIDTH: 64,          // 左右の線の最小長（px）。小さいほど隙間が端に寄れて位置の多様性UP
  // 隙間位置の多様性。連続ステージで中心を最低この距離ずらし、必ず横移動させる（操作の幅を広げる）。
  // 位置（＝左右セグメント比）の変化は難易度を底上げしないので公平性を保ったまま単調さを消せる。
  GAP_CENTER_MIN_SHIFT_PX: 120,        // 連続ステージの隙間中心の最小移動量（px）。0で制約なし
  // 緩急リズム: Nステージ毎に1回、確定で「広い息継ぎ」を挟む（緊張と緩和）。緩和なので常にフェア。
  BREATHER_EVERY_N: 5,                 // このステージ数ごとに息継ぎ（0で無効）
  BREATHER_GAP_MULT: 1.6,              // 息継ぎ時の幅倍率（基準幅×この値）。現状の穴サイズに比例して緩和
  BREATHER_LINE_COLOR: 0x70ff9c,       // 息継ぎステージのライン/マーカー色（緑＝安全/緩和の合図）
  ...
  // --- ライン伸縮（運要素） ---
  // 振幅は「最大値」。実際の振幅はその時のマージン(隙間幅-ボール直径)に連動して縮め、
  // 両ラインが同時に内側へ振れても必ず SAFE_MARGIN だけは通せる量に抑える（公平性担保）。
  // → 余裕のある序盤は大きく脈動し、ギリギリの終盤は自動でほぼ動かなくなる。
  LINE_OSCILLATION_AMPLITUDE_PX: 8,    // ラインの伸縮振幅の最大値（±この値まで）
  LINE_OSC_SAFE_MARGIN_PX: 6,          // 最悪の伸縮時でも必ず残す通過余地（px）。小さいほど攻める
  ...
  // 中立キャリブレーション（持ち方の癖を吸収する基準角の決め方）。
  // 最初の傾きイベント受信を待ってから、落ち着き待ち→短い窓で平均して中立を決める。
  TILT_CALIB_SETTLE_MS: 150,           // 開始直後の手ブレを避ける落ち着き待ち時間
  TILT_CALIB_WINDOW_MS: 250,           // 中立を平均する窓の長さ（単発値の偏りを消す）
  ...
  // shaderトレイルへ渡す速度の平滑時定数(ms)。大きいほど遅れて追従＝バウンス反転時に
  // 平滑速度が一旦0付近を通り、トレイルが急反転せず一瞬縮んでから新方向へ伸び直す。
  // 小さすぎると反転がそのまま出る／大きすぎると落下中もトレイルが遅れて不自然。
  TRAIL_VEL_SMOOTH_MS: 90,
  ...
  // --- デバッグモード（タイトルロゴの隠しタップで起動。開発用） ---
  DEBUG_TAP_COUNT: 10,                 // タイトルロゴを何回タップで debug ON
  DEBUG_GAP_MULT: 1.8,                 // debug時の初期隙間幅の倍率（広くして死ににくく）
